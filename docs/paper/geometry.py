...
# sphere
c.stroke(path.circle(0, 0, 1), fillstyle)

# plane
c.stroke(path.rect(-1.3, -2, 2.6, -0.1), fillstyle)

# z-axis
c.insert(text.text(0.17, 1.35, r"$z$"))
c.stroke(path.line(0, -2, 0, 1.5), [ deco.earrow([deco.stroked([deco.stroked.clear])])])
...
c.insert(text.text(-0.34, -1.6, r"$L$"))
c.stroke(path.line(0, -2, 0, -1), [deco.barrow.small, deco.earrow.small, style.linewidth.thick])
