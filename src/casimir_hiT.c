...
void usage(FILE *stream, const char *name)
{
    ...
    fprintf(stream, "\t-p precicion: precision, default: %g\n", PRECISION);
    fprintf(stream, "\t-t threshold: threshold, default: %g\n", THRESHOLD);
    fprintf(stream, "\t-c cores:     cores used for computation, default: 1\n");
}

pthread_t *start_thread(double LbyR, int m, int ldim, double threshold)
{
...
void *logdetD0(void *p)
{
    ...
    casimir_set_threshold(casimir, threshold);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        int c = getopt_long (argc, argv, "x:t:L:l:p:c:h", long_options, &option_index);
    ...
    double *values = xmalloc(ldim*sizeof(double));
    double *EE     = xmalloc(ldim*sizeof(double));
    ...
    while(m < ldim)
    {
        ...
        {
            if(threads[i] == NULL)
                threads[i] = start_thread(LbyR, m++, ldim, threshold);
